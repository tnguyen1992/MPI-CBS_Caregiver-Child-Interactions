...
run([filepath '/../CARE_init.m']);

cprintf([0,0.6,0], '<strong>------------------------------------------------</strong>\n');
cprintf([0,0.6,0], '<strong>Caregiver-Child interaction project</strong>\n');
cprintf([0,0.6,0], '<strong>Export of coherence results</strong>\n');
cprintf([0,0.6,0], 'Copyright (C) 2019, Trinh Nguyen, univie, Daniel Matthes, MPI CBS\n');
cprintf([0,0.6,0], '<strong>------------------------------------------------</strong>\n');

% -------------------------------------------------------------------------
% Path settings
% -------------------------------------------------------------------------
path = uigetdir(pwd,'Select Coherence Data Folder...');                                         % root path to eeg data
...
srcPath = [path 'DualfNIRS_CARE_processedData/'];
srcPath = [srcPath  '05a_wtc/'];

fileList     = dir([srcPath, 'CARE_d*_05a_wtc_*.mat']);                     % determine all avaible sessions
...
for dyad=1:1:numOfFiles
  fileListCopy{dyad} = strsplit(fileList{dyad}, '05a_wtc_');
...
fileList     = dir([srcPath 'CARE_d*_05a_wtc_' sessionStr '.mat']);
fileList     = struct2cell(fileList);
...
for i = 1:1:numOfFiles
  listOfPart(i) = sscanf(fileList{i}, ['CARE_d%d_05a_wtc_'  sessionStr... % generate a list of all available numbers of dyads
                                        '.mat']);
end
...
filepath = fileparts(mfilename('fullpath'));
load(sprintf('%s/../general/CARE_generalDefinitions.mat', filepath), ...     % load general definitions
     'generalDefinitions');
...
fprintf('<strong>Condition selection...</strong>\n');
selection = false;
while selection == false
  fprintf('Available options:\n');
  fprintf('[1] - Export the average over the selected conditions\n');
  fprintf('[2] - Export the a single values for every selected condition\n');
  x = input('Option: ');
  switch x
    case 1
      selection = true;
